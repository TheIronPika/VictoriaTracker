...
window.showManageDetail = (id) => {
    ...
    const bountyHtml = h.bountyActive
        ...
        +     '<div class="msp-field-row"><span class="msp-field-label">Bonus 🌿 Rest Weeks</span><input type="number" class="msp-field-input" min="0" value="' + sv(h.bountyExcuseTokens) + '" placeholder="—" style="width:100%;box-sizing:border-box" onchange="window.updateField(\'' + h.id + '\',\'bountyExcuseTokens\',this.value)"></div>'
        +     '<div class="msp-field-row"><span class="msp-field-label">Bonus ☀️ Fresh Starts</span><input type="number" class="msp-field-input" min="0" value="' + sv(h.bountyStreakResetTokens) + '" placeholder="—" style="width:100%;box-sizing:border-box" onchange="window.updateField(\'' + h.id + '\',\'bountyStreakResetTokens\',this.value)"></div>'
        +   '</div>'
...
function _wrComputeModel(wk, histForStreaks) {
    ...
    const wins = habitsArr.filter(h => !h.excused)
        .map(h => { const c = cur(h), t = getTier(h, c), s = computeStreaksFromHistory(hist, h.id); return { icon: h.icon, name: h.name, t, payout: pay(h, t), streak: s.streak, bountyDollars: h.bountyDollars||0, bountyStars: h.bountyStars||0, bountyExcuseTokens: h.bountyExcuseTokens||0, bountyStreakResetTokens: h.bountyStreakResetTokens||0 }; })
        ...
        <div style="display:flex;align-items:center;gap:12px;padding:11px 12px;border-radius:10px;margin-bottom:8px;background:${TB[w.t]};">
            ...
            <div style="flex:1;">
                ...
                <div style="display:flex;align-items:center;gap:6px;flex-wrap:wrap;">
                    ...
                    ${w.bountyDollars > 0 || w.bountyStars > 0 || w.bountyExcuseTokens > 0 || w.bountyStreakResetTokens > 0 ? `<span style="font-size:9px;font-weight:700;color:#f0c040;background:rgba(240,192,64,0.12);border:1px solid rgba(240,192,64,0.3);padding:1px 6px;border-radius:8px;">🏆 Bounty${w.bountyDollars > 0 ? ' +$'+w.bountyDollars.toFixed(2) : ''}${w.bountyStars > 0 ? ' ✨'+w.bountyStars : ''}${w.bountyExcuseTokens > 0 ? ' 🌿'+w.bountyExcuseTokens : ''}${w.bountyStreakResetTokens > 0 ? ' ☀️'+w.bountyStreakResetTokens : ''}</span>` : ''}
                </div>
            </div>
...
function buildVictoriaReportHtml(habitsArr, totalBalance, weekEndingOverride, starsEarnedOverride) {
    ...
    const wins = habitsArr
        .filter(h => !h.excused)
        .map(h => { const c = cur(h), t = getTier(h, c), s = computeStreaksFromHistory(state.weeklyHistory, h.id); return { icon: h.icon, name: h.name, t, payout: pay(h, t), streak: s.streak, bountyDollars: h.bountyDollars || 0, bountyStars: h.bountyStars || 0, bountyExcuseTokens: h.bountyExcuseTokens || 0, bountyStreakResetTokens: h.bountyStreakResetTokens || 0, bountyNote: h.bountyNote || '' }; })
        .filter(a => a.t === 'goal' || a.t === 'bonus')
        .sort((a, b) => tierOrder[b.t] !== tierOrder[a.t] ? tierOrder[b.t] - tierOrder[a.t] : b.payout !== a.payout ? b.payout - a.payout : b.streak - a.streak);
