...
- **Water tracker** — daily ounce logging with an animated fill vessel (glass, bottle, tumbler, wine, pool); auto-fills the "Drink Water" habit when the daily goal is hit
...
- **Three token types** — Rest Week 🌿 (skip a habit's penalties for the week), Fresh Start ☀️ (clear a bad-streak counter), Day Pass 🎫 (synthetically count one extra completion)
...
- **Category-wide payouts** — when every active habit in a category hits the same tier, the whole category pays an extra bonus
- **Achievements** — permanent badges unlocked for milestones like streak lengths, earnings totals, water goals, and shop usage; viewable in Manage → Achievements
- **Per-habit star badge** — tappable ⭐ badge on each habit card showing the star breakdown for that habit
...
```
User taps bubble  (or taps the water tracker card)
       │
       ▼
...
Core/ modules (pure logic, no UI)
   ...
   • state.js         — in-memory app state (habits, stars, history, plans, calendar events,
                         waterData, achievements, resetState, categoryConfig, etc.)
   • habits-data.js   — reads/writes Firestore, triggers re-render on change;
                         ...
                         habit.history[] stores PER-DAY counts;
                         weekTotal()/toCumulative() in habits.js convert for display
   • weeklyReset.js   — the reset logic: propose + execute phases (shared
                         between GitHub Action and in-app approval flow)
   • resetState.js    — reads/watches system/reset_state; effectiveDate() pins the
                         UI to last week while a reset is pending; snooze logic (2×15min)
   • stars.js         — star balance, all three token types, shop items
   • water.js         — daily ounce logging; field-level increments prevent tap loss on
                         concurrent devices; auto-fills linked "Drink Water" habit
   • achievements.js  — permanent badge store; unlockAchievement() is idempotent
   ...
   • category-payouts.js  — category-wide payout math (pure, no Firebase import)
   • category-config.js   — Firestore load/watch/save for category_config doc
       │
       ▼
web/ui/ modules (DOM rendering)
   • render.js        — rebuilds the screen after every data change; tab nav;
                        ...
                        renders tappable ⭐ star badge per habit card
   • habits-ui.js     — bubble toggle, lucky draw (tier-scaled odds), token flows
   • water-ui.js      — water tracker card (above sections on Today tab): animated
                        SVG vessel fill, tap to add/undo 10oz, streak display
   • vessel-geometry.js — SVG path data for the five vessel shapes (pure data, no DOM)
   • achievements-ui.js — achievement unlock checks + Manage → Achievements badge grid
   • achievement-catalog.js — static catalog of badge definitions (IDs match native app)
   ...
   • manage-ui.js     — settings split-panel + weekly report popup + forecast;
                        sections: Habits · Add Habit · Events · Stars · Period ·
                        Layout · Streak $ · Achievements · Category
   • animations.js    — time-of-day accent color; Customize Vibe override;
                        perfect-week + streak milestone achievement unlocks + confetti
...
Firebase Firestore (cloud database)
   • 13 documents in the "system" collection
...
Every Monday, GitHub Actions runs scripts/reset.js TWICE:
       ...
       └─ 7:00 PM Central (00:00 UTC Tue) — FORCE mode
              ...
              • applies category-wide bonus payouts (via Core/category-payouts.js)
              • awards stars, bounties, and tokens
              • applies streak bonuses/penalties; advances cycleNextDue for cyclic habits
              • saves a weekly snapshot to history (includes category results)
...
Open the app → tap **History** → tap **MANAGE 🔒** → enter the passcode **1234** → select a habit from the left panel, or click **+ Add Habit**. From there you can edit thresholds, payouts, streak bonuses, cycle type, period sensitivity, and more.

### Use the Plan view

Tap the **Weekly** tab → tap the **Plan** button (sub-tab toggle). Each row is a habit; each column is a day (Mon–Sun). Tap a bubble to mark that you're planning to do the habit on that day. Bubbles are colored by which performance tier the running planned count would reach on each day. Use **Copy Last Week** to carry over last week's plan.

If Google Calendar events have been synced, the week's events appear below the grid as a scrollable agenda, and days with events show density bars under their column headers.

### Use the water tracker

The water card sits at the top of the Today tab. Each tap adds 10 oz; long-press or tap again while full to undo. The vessel fills as ounces increase. To change the vessel shape, tap the vessel icon and pick from glass, bottle, tumbler, wine, or pool float — saved per device. When the daily goal is reached (default 120 oz), the linked "Drink Water" habit bubble is automatically filled for today; falling back under goal removes it.

### View achievements

Open **Manage** (History tab → MANAGE 🔒 → passcode `1234`) → click **Achievements** in the left nav. Unlocked badges appear with their icon, label, and unlock date. Locked badges are shown dimmed.

### Reorder today-view sections

Open the Manage panel (History tab → MANAGE 🔒 → passcode `1234`) → click **Layout** in the left nav → use the up/down arrows to rearrange categories, Seasonal Events, and Room Checks. The order syncs live across all devices. The water tracker card is always first — it is not part of the section-order system.

### Configure category-wide payouts

Open **Manage** → **Category** in the left nav. Add a dollar/star/token reward for each category tier. When every active habit in a category hits that tier for the week, the category pays the configured reward on top of the individual habit payouts.

### Customize the accent color (Vibe)

Tap **History** → tap the **⚙** tab → drag the **Hue** and **Intensity** sliders under "Customize the Vibe". The choice is saved to `localStorage` and persists across sessions on that device. Tap **Reset to auto** to return to the time-of-day color animation.

### Enable a daily reminder

Tap **History** → tap the **⚙** tab → check **Enable daily reminder** and set a time. The app will fire a browser notification at that time on any day it's open. Permission must be granted in the browser the first time. The setting is per-device.
...
**Check:** GitHub → Actions tab → see if the Monday runs are there and what their status was. There are two runs: the "propose" at ~4am and the "force" at ~7pm (both Central time).
...
**Fix:** Re-run the workflow manually from the Actions tab using `force` mode. If it fails, look at the logs for the specific error. If you need to re-run a reset that technically already ran today, add `FORCE_RESET=1` as an environment variable in the workflow dispatch.
...
The app's "This Week's Balance" display and the reset script both call `computeWeeklyPayout()` from `Core/habits.js`. Divergence only happens if logic was moved out of `habits.js` without updating callers.

**Fix:** Check `Core/habits.js` `computeWeeklyPayout()` and verify `render.js`, `manage-ui.js`, and `Core/weeklyReset.js` all call the same function. Also verify `Core/category-payouts.js` `computeCategoryResult()` is used consistently in all four places that show a dollar total.

### 4. Water tracker shows wrong amount or habit doesn't auto-fill

**Common causes:**
- Concurrent tap writes that got clamped: the stored value can briefly go negative from a race, but `normalizeWaterData()` floors it at 0 on the next load.
- `WATER_CONFIG.linkedHabitId` in `Core/config.js` doesn't match the actual "Drink Water" habit's `id` field in Firestore.
- The reset is pending (Monday before 7pm): `effectiveDate()` returns last Sunday, so day index 6. Water sync correctly targets that index. If the UI looks frozen on the old week, that's expected behavior until the reset executes.

**Fix:** Check the browser console for `syncWaterHabit` errors. Verify the `linkedHabitId` matches by opening Manage → Habits and checking the habit's `id`.

### 5. Google Calendar events not showing in the Plan view

**Common causes:**
- `GOOGLE_CALENDAR_CONFIG.clientId` is empty in `Core/config.js` — integration is disabled
- The device has not previously connected (no `localStorage.gcalConnected` flag) — there is currently no connect button in the UI; to force a sync call `window.connectGoogleCalendar()` from the browser console
...
### 6. Reset propose/force flow confusion

The "propose" run at 4am only flips a flag (`pendingReset = true`) — it doesn't score anything. If you open the app between 4am and 7pm on Monday and see a pending reset prompt, that's expected. The actual scoring happens either when Victoria approves in the app or when the "force" run fires at 7pm. While a reset is pending, `effectiveDate()` pins the app to show last week's data — this is intentional so she can review and correct before the final tally.
...
| **Bounty** | A one-time bonus (dollars, stars, and/or tokens) set on a habit that fires when she hits Goal/Bonus that week, then clears automatically |
...
| **Rest Week** 🌿 | Token (formerly "Excuse") — freezes one habit for one week (no payout either direction, streak held); internal field: `excuseTokens` |
| **Fresh Start** ☀️ | Token (formerly "Streak Reset") — zeroes a habit's bad-streak counter without requiring a goal week; internal field: `streakResetTokens` |
| **Day Pass** 🎫 | Token (formerly "Mark Off") — synthetically adds +1 completion to a habit for the current day (appears as a grey bubble); internal field: `markOffTokens` |
| **Category payout** | A bonus that fires when every active habit in a category hits the same tier; configured per-tier in Manage → Category |
| **Achievement** | A permanent badge unlocked for a milestone (streak length, earnings total, water goal, etc.); displayed in Manage → Achievements |
| **Water tracker** | Daily ounce-logging card on the Today tab; auto-fills the linked "Drink Water" habit bubble when the daily goal is reached |
...
| **effectiveDate** | The date the UI treats as "today" — normally the real date, but pinned to Sunday of the pending week while a reset is overdue |
...
| **Manage panel** | The admin/settings section of the app, behind passcode `1234`; accessed from the History tab via the "MANAGE 🔒" button; contains sub-sections: Habits, Add Habit, Events, Stars, Period, Layout, Streak $, Achievements, Category |
...
| **Calendar events** | Events fetched from Google Calendar and shown in the Planning agenda; synced to Firestore so they're visible across devices |
