...
flowchart TD
    ...
    subgraph GHP["GitHub Pages — static host"]
        ...
        subgraph UI["web/ui/ — DOM rendering"]
            direction TB
            RENDER["render.js<br/>main render loop · tab nav"]
    ...
    subgraph CDN["CDNs — runtime dependencies"]
        ...
        FONTS["Google Fonts<br/>Playfair Display · Montserrat"]
    end

    subgraph FIREBASE["Firebase — Firestore"]
        direction TB
        FS["Firestore DB<br/>project: victoria-tracker-1d2ab"]
        DOCS["system/ collection<br/>habits_list · weekly_history<br/>star_data · seasonal_events<br/>period_data · rooms_data · reset_state"]
    end

    subgraph GHA["GitHub Actions — weekly automation"]
        direction TB
        CRON["weekly-reset.yml<br/>cron: Mon 09:00 UTC"]
        RESET["scripts/reset.js<br/>Node 22 · node-fetch · REST API"]
    end
    ...
    LS["localStorage<br/>UI state: collapsed sections<br/>sort lock · filter mode<br/>report mute flag per device"]
    ...
    CRON -->|"triggers"| RESET
    RESET -->|"Firestore REST API<br/>resets counters · saves snapshot<br/>awards stars"| FS
    RESET -->|"sends report email"| EMAILJS
    ...
    class USER,IDX,SW,MAN,CFG,STATE,FBWRAP,HAB,FEAT,HIST,REP,RENDER,UIMOD app;
    class FS,DOCS,LS store;
    class FSDK,CHARTJS,SORTABLE,CONFETTI,EJSCDN,FONTS,EMAILJS,OWM,OPENUV ext;
    class CRON,RESET auto;
```

**Legend.** Blue nodes are the app's own code: `index.html` (the sole entry point, which also hosts the interactive weekly report popup overlay), the `Core/` ES modules that handle all data and business logic, and the `web/ui/` modules that render the DOM. Green nodes are data stores: Firestore (cloud, seven documents in the `system` collection) and `localStorage` (browser, holding UI preferences and per-device report mute flags). Pink nodes are everything the app calls at runtime from the outside world — CDN libraries loaded into the browser, Firebase's own SDK, the EmailJS relay service, and the OpenWeatherMap and OpenUV APIs that power the header weather cards. Purple nodes are the automated weekly reset: a GitHub Actions cron job that runs `scripts/reset.js` every Monday at 09:00 UTC, scoring the previous week via the Firestore REST API (not firebase-admin), awarding stars, saving a history snapshot, resetting all habit counters, and sending the weekly email report — all without anyone opening the app. On the next app open after a reset, the browser detects the new reset date and auto-shows the interactive report popup once per device.
