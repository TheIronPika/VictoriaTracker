...
**Note:** The EmailJS browser SDK is still loaded in `index.html` and `EMAIL_CONFIG` is defined in `config.js`, but `emailjs.send()` is not currently called anywhere — the weekly email feature is inactive as of 2026-07-17.
...
│   │                       also hosts the weekly report popup + forecast computation
...
**Most important entry point:** `index.html` — it wires everything together: imports all Core and UI modules, starts the Firestore `watchHabits()` listener, kicks off animations/weather, and subscribes to live updates for plans and calendar events. Also loads `section-order.js` for the draggable Today-view layout.

**Most important reset file:** `Core/weeklyReset.js` — contains the single implementation of the Monday reset logic (payouts, star awards, streak updates, history snapshot, room/event reset, cycle advancement). This is the file to edit if reset behavior needs to change.

**Tab structure (as of 2026-07-21):**
- Tab 0 — **Today**: habit cards, bubble toggle, room checks, seasonal events
...
5. **The weekly reset is a two-phase propose → execute flow.** `Core/weeklyReset.js` exports two functions:
   ...
   - `executeWeeklyReset(io, now)` — the actual reset (payouts, snapshots, wipe, etc.). Called either when Victoria approves in-app (planned) or by GitHub Actions force mode at 7pm Central Monday.
...
6. **Period protection skips scoring, not history.** When a period is active, period-sensitive habits keep logging completions normally but the reset ignores their payouts/penalties. Bubbles turn pink for those days. When `periodWasThisWeek` is true (ended mid-week), those habits' streaks are also frozen on reset. Both starting and ending a period support backdating: the date-strip day selector is used as the effective start/end timestamp; `endPeriod()` sets 23:59:59 on that day when backdating. The modal title reflects the selected date (e.g., "End period on Thursday, Jul 10") when a past date is chosen.
...
10. **`Core/` modules must never import from `web/ui/`.** The dependency arrow is one-way: UI imports Core, never the reverse. This keeps Core testable outside a browser (and specifically makes `Core/weeklyReset.js` runnable from the GitHub Action).

11. **Manage panel passcode is `1234`** (see `MANAGE_PASSCODE` in `config.js`). It's intentionally public since this is a single-user personal app. Access: History tab → "MANAGE 🔒" button → enter passcode.

12. **Service worker caches the app shell only.** Firestore, CDN libraries, and API calls always go network-first. The app shows stale UI when offline but won't lose data. **When adding a new file to the app shell, add it to the `SHELL` array in `sw.js` and bump the `CACHE` version string.**
...
17. **The Planning view stores intent, not history.** `Core/planning.js` and `web/ui/planning-ui.js` let Victoria pre-fill which days she plans to do each habit. This data lives in `system/weekly_plans` and **never touches `habit.history`**. It is purely visual — the Plan view has no effect on payouts or streaks. Only the last 16 weeks of plans are retained in Firestore to prevent unbounded growth. The Plan view is a sub-tab within the Weekly tab (`switchWeeklySub('plan')`), not a separate top-level tab.
...
22. **History > Settings sub-tab (⚙).** Contains three user-facing controls:
