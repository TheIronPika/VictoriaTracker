...
6. **`Core/resetState.js` owns the UI-side reset approval flow.** It is a separate module from `Core/weeklyReset.js` (which is shared with the GitHub Action and must not grow browser-only deps):
   ...
   - **The web UI is pinned too, as of 2026-08-06.** Until then only `water.js` actually called `effectiveDate()` — the docs claimed the whole UI was pinned, but `render.js` and `habits-ui.js` still derived "today" from `new Date()`. That meant a Monday tap in the browser wrote `history[0]` of the *new* week onto last week's un-scored Monday, and the 7pm force-run then paid out and permanently snapshotted the tampered week. Now pinned in four places: `render()` snaps `uiState.viewingDate` into the effective week (week-level only, so a same-week date-strip pick is untouched), `buildDateStrip()` builds from the effective week, and `render.js`'s bubble `todayIdx` plus `habits-ui.js`'s `todayIdx` / future-day guards all read `getDayIdx(effectiveDate())`. **Never reintroduce a bare `getDayIdx(new Date())` in the habit UI** — that is the whole bug.
...
26. **Category-wide payouts (`system/category_config`).** The only cross-habit payout in the app: when every *counting* habit in a category reaches a tier, the category itself pays dollars, stars and all three tokens.
    - **The tier rule (revised 2026-08-06 — ONE FORGIVEN STRAGGLER):** a category reaches a tier when every counting habit got there, **except that a single habit sitting exactly one tier below is forgiven**. 6 at Bonus + 1 at Goal → **Bonus**; 5 at Bonus + 2 at Goal → **Goal**; 6 at Bonus + 1 at Low → **Goal** (two rungs down can't be forgiven at Bonus). The category takes the highest rung that qualifies. Two guards: only the *closest* habit below the tier is pardoned and only at exactly one rung down, and **at least one habit must actually be at or above the tier** (otherwise a lone habit at Goal would forgive itself into Bonus). Replaces the old strict-minimum rule, which produced Debt for all five categories in all 15 stored weeks.
    - **Period protection suspends forgiveness for categories in `STRICT_WHEN_PERIOD_PROTECTED` (currently `['Sexual']`).** Protected habits leave the counting set entirely, which can shrink a category enough that forgiveness becomes disproportionate: Sexual has 6 counting habits normally but only **2** during a period, so one forgiven straggler would let a single habit at tier carry the whole payout. Protection exists so she isn't *penalised* that week, not to make the reward markedly cheaper. Keyed on protection having actually removed a habit from **that** category (`protectedOut > 0`), not merely on a period being active. `computeCategoryResult` returns `protectedOut` and `forgive` so the UI can explain itself and `blockersForRank` can agree with the tier awarded. ⚠️ Matched on the `cat` string — renaming the category silently disables it.
    - **`blockersForRank(counting, rank, { forgive })` is the one source of "N to go"** — it already excludes the habit that rung would forgive, so `.length` is a truthful count of moves required. `laggards` is keyed to the *next* rung, not the achieved one. Never count "habits below rank" by hand; that was wrong for any rung past the next one.
