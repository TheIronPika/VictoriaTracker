...
26. **Category-wide payouts (`system/category_config`).** The only cross-habit payout in the app: when every *counting* habit in a category reaches a tier, the category itself pays dollars, stars and all three tokens.
    ...
    - **`blockersForRank(counting, rank)` is the one source of "N to go"** — it already excludes the habit that rung would forgive, so `.length` is a truthful count of moves required. `laggards` is keyed to the *next* rung, not the achieved one. Never count "habits below rank" by hand; that was wrong for any rung past the next one.
